...
Supplier Prequalification is the entry-gate module for S.M.A.R.T. Hawk's supply-chain quality program — every supplier that ships material into a regulated workflow walks the five pillars at least once, and re-walks them on the periodic requalification cadence. **Sense** happens when procurement nominates a new supplier and the `supplierIntelAgent` fuses public data from FDA, EMA EudraGMDP, and WHO-PQ (Warning Letters, inspection history, FEI registrations) into a starter dossier; the supplier then completes a `PrequalQuestionnaire` (PQQ) capturing capabilities, certifications, and past audit history. **Monitor** normalizes the submission into `Supplier` + `SupplierSite` + `SupplierProduct` + `ProductSiteMapping` records and resolves identity against the existing supplier-master; a `SupplierQualification` case is opened with state machine tracking the lifecycle. **Analyze** runs in two layers: QA Prequal performs a desk review (cert validity, regulatory standing, completeness of evidence) via `DeskReviewDisposition` rows, and `supplierScoreService` computes a FMEA-weighted risk score; if the score warrants it, `siteVisitTriggerService` hands off to the Audit Management module to schedule an on-site visit. **Record** ends with the QA Approver issuing an Approved / Conditionally Approved / Not Qualified decision via APPROVED e-signature; the supplier moves to the approved list and `requalScheduler` books the next periodic requalification. **Trace** writes an `AuditTrail` row per qualification event, score change, disposition, and approval signature; the case is immutable, and downstream modules (Audit Management, Batch Records, Marketplace) consume the approved-supplier status as a gating eligibility check.
...
- **FEEDS** Marketplace — qualified suppliers become listable in the S.M.A.R.T. Hawk marketplace module (downstream eligibility gate)
...
|---|---|---|
| 1 · Sense | `backend/src/controllers/supplierController.js` · `models/Supplier.js` | Nomination + supplier master record |
| 1 · Sense | `backend/src/controllers/prequalQuestionnaireController.js` · `models/PrequalQuestionnaire.js` | PQQ template instantiation and supplier-side data capture |
| 1 · Sense | `backend/src/services/ai/supplierIntelAgent.js` | Public-data fusion from FDA · EMA EudraGMDP · WHO-PQ |
| 2 · Monitor | `backend/src/controllers/qualificationController.js` · `models/SupplierQualification.js` · `SupplierSite.js` · `SupplierProduct.js` · `ProductSiteMapping.js` | Opens qualification case + normalizes site/product records |
| 2 · Monitor | `backend/src/services/supplierQualificationService.js → canTransition()` | State-machine gating per `constants/supplierStates.js` |
| 3 · Analyze | `backend/src/controllers/deskReviewController.js` · `models/DeskReviewDisposition.js` | Per-section desk review dispositions by QA Prequal |
| 3 · Analyze | `backend/src/services/supplierScoreService.js` · `models/SupplierScore.js` | FMEA-weighted scoring with snapshot weights per case |
| 3 · Analyze | `backend/src/services/siteVisitTriggerService.js` | Hands off to Audit Management when on-site verification is warranted |
| 4 · Record | `backend/src/controllers/qualificationApprovalController.js` · `middlewares/requireESignature.js` (`signatureMeaning='APPROVED'`) | G-APR approval / rejection / disqualification e-sig by QA Approver |
| 4 · Record | `backend/src/cron/requalScheduler.js` | Daily cron books next PERIODIC_REQUAL case based on `qualifiedUntil` |
| 5 · Trace | `backend/src/services/auditTrailService.js` (shared) | AuditTrail per qualification event · score change · state transition · sign-off |
| 5 · Trace | `Supplier.status` + `qualifiedUntil` fields consumed by Audit · Batch Records · Marketplace | Downstream modules treat approved status as the gating eligibility check |
